...
impl MessageHandler<UserCreatedEventMessage> for UserCreatedHandler {
    fn handle(&self, message: Box<UserCreatedEventMessage>
        ...
        println!("In Abi Naufal's Computer [2406361694]. Message received: {:?}", message);
        Ok(())
    }
}

fn main() {
    let amqp_url = std::env::var("AMQP_URL").unwrap_or_else(|_| "amqp://guest:guest@localhost:5672".to_owned());
